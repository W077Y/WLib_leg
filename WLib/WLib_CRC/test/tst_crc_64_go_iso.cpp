...
TEST_CASE("tst_crc64_go_iso")
{
  ...
  uint64_t crc =
      WLib::CRC::CRC_64_go_iso()(reinterpret_cast<std::byte const*>(tst_str), std::size(tst_str));
...
TEST_CASE("tst_crc64_go_iso string")
{
  ...
  uint64_t crc = WLib::CRC::CRC_64_go_iso()(reinterpret_cast<std::byte const*>(tst_str),
                                            std::size(tst_str) - 1);
